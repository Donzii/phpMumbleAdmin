	...
	*/
	private function channel( $obj, $deep, $last_id ) {
		...
		if ( $chan->id > 0 ) {
			...
			$this->output .= HTML::img( 'pma/'.$img.'.png' );
		}
		...
		$this->output .= HTML::img( $img.'.png' );

		// Space padding
		$this->output .= HTML::img( IMG_SPACE_16 );
...
// 		}
	...
	private function get_chan_password_icon( $id ) {
		...
		foreach ( $aclList as $acl ) {
			...
			if ( ! $acl->inherited && PMA_helpers_ACL::is_token( $acl ) ) {
				...
				$this->output .= HTML::img( 'gei/padlock_16.png' );
				break;
			}
		}
	}
	...
	*/
	private function get_chan_status_icons( $chan ) {
		...
		// Space padding
		$this->output .= '<span class="block right">';
		...
		if ( $chan->temporary ) {
			$this->output .= HTML::img( 'tango/clock_16.png' );
		}

		if ( $chan->description !== '' ) {
			$this->output .= HTML::img( 'mumble/comment.png' );
		}

		if ( $this->default_channel_id === $chan->id ) {
			$this->output .= HTML::img( 'other/home_16.png' );
		}
	...
	*/
	private function user( $user, $deep, $last_sessid, $sub_channels ) {
		...
		// User icon
		$this->output .= HTML::img( 'pma/tree_user.png' );
	...
	private function get_user_status_icons( $user ) {
		...
		if ( $user->userid >= 0 ) {
			$this->output .= HTML::img( 'mumble/user_auth.png' );
		}
		if ( isset( $user->recording ) && $user->recording ) {
			$this->output .= HTML::img( 'xchat/red_16.png' );
		}
		if ( isset( $user->prioritySpeaker ) && $user->prioritySpeaker ) {
			$this->output .= HTML::img( 'tango/microphone_16.png' );
		}
		if ( $user->comment !== '' ) {
			$this->output .= HTML::img( 'mumble/comment.png' );
		}
		if ( $user->suppress ) {
			$this->output .= HTML::img( 'mumble/user_suppressed.png' );
		}
		if ( $user->mute ) {
			$this->output .= HTML::img( 'mumble/user_muted.png' );
		}
		if ( $user->deaf ) {
			$this->output .= HTML::img( 'mumble/user_deafened.png' );
		}
		if ( $user->selfMute ) {
			$this->output .= HTML::img( 'mumble/user_selfmute.png' );
		}
		if ( $user->selfDeaf ) {
			$this->output .= HTML::img( 'mumble/user_selfdeaf.png' );
		}
